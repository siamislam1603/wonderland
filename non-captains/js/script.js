...
$(".js-sort").on("click", function () {
  ...
  if (item === null) {
    // Get keys with the minimum count
    const minCount = Math.min(...Object.values(houseMemberCount));
    const keysWithMinCount = Object.keys(houseMemberCount).filter(
      (key) => houseMemberCount[key] === minCount
    );

    // Randomly pick one key from the keys with the minimum count
    const randomlyPickedKey =
      keysWithMinCount[Math.floor(Math.random() * keysWithMinCount.length)];

    console.log(keysWithMinCount, randomlyPickedKey);

    // const timer = setTimeout(() => {
    //   if (showCurrentMemberHouse) saveMemberHouse(randomlyPickedKey);
    //   clearTimeout(timer);
    // }, 1000);
    while (true) {
      item = randomlyPickedKey;
      if (randomlyPickedKey) {
...
function statusOne() {
  ...
  setTimeout(() => {
    ...
    if (setNextMember()) {
      ...
      $(".status-1 div.sorting-hat,.avatar__mouth").removeClass("animate");
    }
  }, 1000);
}
